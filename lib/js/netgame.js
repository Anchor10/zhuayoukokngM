...
$(window).bind('scroll', function() {
  ...
  function showDown3(channelId) {
    ...
    toUrl = contextRoot + '/e/ikaimi/m/loaddown.bt.php?id=' + channelId + "&" + publicArg;
    $.ajax({
      url : toUrl, type : 'GET', cache : false, dataType : 'text', success : function(data) {
        $(".gmdetaDown").remove();
        var downloadFloat = $(data);
  ...
  $(function() {
    ...
    $('.userBtn2').on('click', hideUser);
    $('.logbg').on('click', hideUser);
    function hideUser() {
      $('.userInfo').animate({
        'left' : "100%"
      }, function() {
        $('body').unon('touchmove', stopScroll);
        $('.logbg').hide();
      });
      bar.animate({
        'right' : '-100%'
